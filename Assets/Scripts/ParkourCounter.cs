...
public class ParkourCounter : MonoBehaviour
{
    ...
    // jumps of the last roll
    public int lastRoll;
    // state of the t-shape interaction (0: not interacting, 1: left-right, 2: up-down or 3: turning)
    public int interacting;

    // counter for the reacting minigame
    public int reactionCount;
    // timer for the reaction minigame
    ...
    void Update()
    {
        if (isStageChange)
        {
            ...
            else if (locomotionTech.stage == firstBanner.name)
            {
                ...
                UpdateRecordText(1, part1Time, part1Count, 18);
            }
            else if (locomotionTech.stage == secondBanner.name)
            {
                ...
                UpdateRecordText(2, part2Time, part2Count, 15);
            }
            else if (locomotionTech.stage == finalBanner.name)
            {
                ...
                UpdateRecordText(3, part3Time, part3Count, 12);
                ...
                endTextGO.GetComponent<TMP_Text>().text = "Parkour Finished!\n" + recordText.text +
                    "\ntotal: " + timeCounter.ToString("F1") + ", " + coinCount.ToString() + "/45";
        ...
        // shows the number of available jumps 
        jumpText.text = "jumps: " + availableJumps.ToString();
    }
